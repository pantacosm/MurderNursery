...
public class ReplaceRelationshipText : MonoBehaviour, IPointerClickHandler
{
    ...
    OpenRelationshipOptionsPanel storyObj;

    [SerializeField]
    Transform scarletContent;
    ...
    private void Start()
    {
        ...
        storyObj = scarletContent.GetComponent<OpenRelationshipOptionsPanel>();
    }
    ...
    //void UpdateStoryText(string storyToUpdate)
    //{
    //    foreach (Transform item in content)
    //    {
    //        if(content.childCount > 0)
    //        {
    //            if(item.name == storyToUpdate)
    //            {
    //                foreach (Transform childItem in item)
    //                {
    //                    if(childItem.name == "One")
    //                    {
    //                        textToReplace = childItem.GetComponent<TextMeshProUGUI>().text;
    //                        childItem.GetComponent<TextMeshProUGUI>().text = childItem.GetComponent<TextMeshProUGUI>().text.Replace(textToReplace, replacingText);
    //                    }
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        if (scarletContent.Find("One"))
        {
            obj = scarletContent.Find("One").gameObject;
            if (obj == storyObj.objToReplace)
            {
                if (storyObj.name == "One")
                {
                    textToReplace = obj.GetComponent<TextMeshProUGUI>().text;
                    obj.GetComponent<TextMeshProUGUI>().text = obj.GetComponent<TextMeshProUGUI>().text.Replace(textToReplace, replacingText);
                }
            }
        }
        if (scarletContent.Find("Two"))
        {
            obj = scarletContent.Find("Two").gameObject;
            if (obj == storyObj.objToReplace)
            {
                if (storyObj.name == "Two")
                {
                    textToReplace = obj.GetComponent<TextMeshProUGUI>().text;
                    obj.GetComponent<TextMeshProUGUI>().text = obj.GetComponent<TextMeshProUGUI>().text.Replace(textToReplace, replacingText);
                }
